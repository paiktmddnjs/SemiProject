            ...
            <c:choose>
                <c:when test="${not empty transactionList}">
                    <c:forEach var="item" items="${transactionList}">
                        <%-- 'INCOME'/'EXPENSE' 또는 '수익'/'지출' 모두 처리 --%>
                        <c:set var="isProfit" value="${item.financialType eq '수익' or item.financialType eq 'INCOME'}" />
                        <c:set var="displayType" value="${isProfit ? '수익' : '지출'}" />
                        ...
                        <tr>
                            ...
                            <td><span class="tag ${tagClass}">${displayType}</span></td>
                            <td>${item.category}</td>
