...
.tab-button {
    ...
    border-bottom: 3px solid transparent; /* 비활성화 시 투명한 밑줄 */
}
...
.tab-button.active {
    color: #ff9800; /* 활성 탭 색상 */
    font-weight: bold;
    border-bottom: 3px solid #ff9800; /* 활성 탭 밑줄 */
}

/* 기존 테이블 스타일 (기타 스타일은 유지한다고 가정) */
.table-card {
...
<div class="container">
    ...
    <div class="table-card">
        ...
        <%-- 탭 메뉴 영역 추가 --%>
        <div class="tab-menu">
            <button class="tab-button <c:if test="${selectedCategory eq '전체'}">active</c:if>"
                    onclick="filterContent('전체')">전체</button>
            <button class="tab-button <c:if test="${selectedCategory eq '리뷰'}">active</c:if>"
                    onclick="filterContent('리뷰')">리뷰</button>
            <button class="tab-button <c:if test="${selectedCategory eq '브이로그'}">active</c:if>"
                    onclick="filterContent('브이로그')">브이로그</button>
            <button class="tab-button <c:if test="${selectedCategory eq '튜토리얼'}">active</c:if>"
                    onclick="filterContent('튜토리얼')">튜토리얼</button>
            <button class="tab-button <c:if test="${selectedCategory eq '소통'}">active</c:if>"
                              onclick="filterContent('소통')">소통</button>
            <button class="tab-button <c:if test="${selectedCategory eq '엔터테인먼트'}">active</c:if>"
                                onclick="filterContent('엔터테인먼트')">엔터테인먼트</button>
        </div>
        ...
        <div class="table-container">
            <table class="content-table">
                ...
                <c:forEach var="content" items="${contentList2}">
                    ...
                    <%-- ⭐⭐ 여기에 탭 필터링 로직 추가 ⭐⭐ --%>
                    <c:set var="contentCat" value="${content.category}" />
                    <c:set var="isAll" value="${selectedCategory eq '전체'}" />
                    <c:set var="isMatch" value="${selectedCategory eq contentCat}" />

                    <c:if test="${isAll or isMatch}">
                                ...
                                <div class="platform-badge">
                                    <div class="platform-icon ${content.platformIcon}">
                                        <c:choose>
                                            ...
                                            <c:when test="${fn:toLowerCase(content.platformIcon) eq 'chzzk'}">
                                            ...
                                            <%-- 아이콘이 정의되지 않은 경우 (물음표 아이콘) --%>
                                            <c:otherwise>
                                                <svg width="30" height="30" version="1.1" xmlns="http://www.w3.org/2000/svg" desc="Created with imagetracer.js version 1.2.6" ><path fill="rgb(0,0,0)" stroke="rgb(0,0,0)" stroke-width="1" opacity="0.13725490196078433" d="M 9.5 4 L 8.5 6 L 9.5 4 Z " /><path fill="rgb(0,0,0)" stroke="rgb(0,0,0)" stroke-width="1" opacity="0.13725490196078433" d="M 20.5 4 L 22.5 7 L 20.5 4 Z " /><path fill="rgb(0,0,0)" stroke="rgb(0,0,0)" stroke-width="1" opacity="0.13725490196078433" d="M 16.5 7 L 18 10.5 L 15 12.5 L 17 8.5 L 16.5 7 Z " /><path fill="rgb(0,0,0)" stroke="rgb(0,0,0)" stroke-width="1" opacity="0.13725490196078433" d="M 18.5 15 L 17.5 17 L 18.5 15 Z " /><path fill="rgb(0,0,0)" stroke="rgb(0,0,0)" stroke-width="1" opacity="0.9215686274509803" d="M 17.5 11 L 16.5 13 L 17.5 11 Z " /><path fill="rgb(0,0,0)" stroke="rgb(0,0,0)" stroke-width="1" opacity="0.9215686274509803" d="M 21.5 12 L 20.5 14 L 21.5 12 Z " /><path fill="rgb(0,0,0)" stroke="rgb(0,0,0)" stroke-width="1" opacity="0.9215686274509803" d="M 13.5 22 L 12.5 24 L 13.5 22 Z " /><path fill="rgb(0,0,0)" stroke="rgb(0,0,0)" stroke-width="1" opacity="0.9215686274509803" d="M 15.5 22 L 16.5 24 L 15.5 22 Z " /><path fill="rgb(0,0,0)" stroke="rgb(0,0,0)" stroke-width="1" opacity="0.9215686274509803" d="M 12.5 25 L 13.5 27 L 12.5 25 Z " /><path fill="rgb(0,0,0)" stroke="rgb(0,0,0)" stroke-width="1" opacity="0.9215686274509803" d="M 16.5 25 L 15.5 27 L 16.5 25 Z " /><path fill="rgb(0,0,0)" stroke="rgb(0,0,0)" stroke-width="1" opacity="0" d="M 0 0 L 30 0 L 30 30 L 0 30 L 0 0 Z M 12 3 L 10 4 L 7 10 L 12 10 L 12 8 L 15 7 L 17 9 L 17 11 L 12 16 L 12 19 L 17 19 L 17 18 L 22 14 Q 24 12 23 7 Q 21 1 12 3 Z M 12 22 L 11 25 L 12 27 L 17 27 L 17 26 L 17 24 L 17 22 L 12 22 Z " /><path fill="rgb(0,0,0)" stroke="rgb(0,0,0)" stroke-width="1" opacity="1" d="M 11.5 4 Q 20.3 2.3 22 7.5 Q 23 14 18.5 15 L 16 16.5 Q 17.1 20.7 13 19 Q 12 15.2 14.5 14 L 18 11.5 Q 19.9 6.1 16.5 7 L 14.5 6 L 11 8.5 Q 11.7 11.5 8 10 Q 7.3 5.3 11.5 4 Z " /><path fill="rgb(0,0,0)" stroke="rgb(0,0,0)" stroke-width="1" opacity="1" d="M 13.5 23 L 17 24.5 L 13.5 26 Q 10.5 24.5 13.5 23 Z " /><path fill="rgb(0,0,0)" stroke="rgb(0,0,0)" stroke-width="1" opacity="0.6235294117647059" d="M 22.5 7 L 23 10.5 L 22 10.5 L 22.5 7 Z " /><path fill="rgb(0,0,0)" stroke="rgb(0,0,0)" stroke-width="1" opacity="0.6235294117647059" d="M 14.5 13 L 13.5 15 L 14.5 13 Z " /><path fill="rgb(0,0,0)" stroke="rgb(0,0,0)" stroke-width="1" opacity="0.6235294117647059" d="M 12.5 16 L 13 18.5 L 12 18.5 L 12.5 16 Z " /></svg>
                                            </c:otherwise>
                                        </c:choose>
                                    </div>
   ...
   <script>
       /**
        * 카테고리를 변경하고 서버에 해당 카테고리 데이터 조회를 요청합니다.
        * * @param category 선택된 카테고리 이름 ('전체', '리뷰' 등)
        */
       function filterContent(category) {
           // 탭 전환은 보통 URL 파라미터 변경을 통해 서버에 새로운 요청을 보냅니다.

           // 1. 현재 URL을 가져옵니다.
           const currentUrl = new URL(window.location.href);

           // 2. 'category' 파라미터 값을 설정합니다.
           currentUrl.searchParams.set('category', category);

           // 3. 페이지를 이동하여 서버에서 새로운 카테고리 데이터(contentList2)를 받아오도록 합니다.
           window.location.href = currentUrl.toString();

           // 이 로직이 작동하려면 서버(Spring Controller 등)에서
           // /list?category=리뷰와 같은 요청을 받아 Model에 데이터를 담아 JSP로 포워딩해야 합니다.
       }
   </script>
    </div>
</div>
